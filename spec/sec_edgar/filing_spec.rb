...
describe SecEdgar::Filing do
  ...
  describe '::filings_for_index' do
    ...
    let(:filing1) { SecEdgar::Filing.filings_for_index(index).first }

    it 'parses all of the filings' do
      expect(SecEdgar::Filing.filings_for_index(index).count).to eq(4628)
    end
  ...
  describe '::for_date' do
    let(:filings) do
      [].tap do |filings|
        SecEdgar::Filing.for_date(Date.parse('20121123')) do |f|
          filings << f
        end
      end
    end
    ...
    it 'returns nil if for_date is run on a market close day' do
      expect(SecEdgar::Filing.for_date(Date.parse('20120101'))).to eq(nil)
    end
  end
  ...
  describe '::recent', vcr: { cassette_name: 'recent' } do
    ...
    before(:each) do
      SecEdgar::Filing.recent(start: 0, count: 10, limit: 10) do |filing|
        filings.push filing
      end
    end
    ...
    it 'should have filing attributes', vcr: { cassette_name: 'recent' } do
      filings.each do |filing|
        expect(filing.cik).not_to be_nil
        expect(filing.title).not_to be_nil
        expect(filing.summary).not_to be_nil
        expect(filing.link).not_to be_nil
        expect(filing.term).not_to be_nil
        expect(filing.date).not_to be_nil
        expect(filing.file_id).not_to be_nil
      end
    end
  end
  ...
  describe "::find" do
    ...
    # let(:entity) {SecEdgar::Entity.find(query[:cik])}
    #
